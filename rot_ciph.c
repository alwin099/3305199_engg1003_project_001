...
*  Date: 29/4/19
*  Description: Encrypts and decrypts a message using the substitution cipher (Ceaser cipher) method.*/
...

{
    ...
    case 1:
    {
        ...
        scanf("%99s", message); // store message as a string in the variable message.
    ...
    case 2:
        ...
        for (i = 0; (i < 100 && message[i] != '\0'); i++)
        {
            message [i] = message[i] - key; //decrypts the message by subtracting the particluar key from thr ASCII value
            if (message[i] > 'z')
               message[i] = message[i] - 26;
        }
